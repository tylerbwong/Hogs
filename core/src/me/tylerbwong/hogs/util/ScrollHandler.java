...
public class ScrollHandler {
   ...
   public ScrollHandler(HogWorld world, float yPos) {
      this.world = world;
   ...
   public boolean collides(Hog hog) {
      if (!pipe1.isScored()
            && pipe1.getX() + (pipe1.getWidth() / 2) < hog.getX()
            + hog.getWidth()) {
         addScore(1);
         ...
         AssetLoader.coin.play();
            ...
            && pipe2.getX() + (pipe2.getWidth() / 2) < hog.getX()
            + hog.getWidth()) {
         addScore(1);
         ...
         AssetLoader.coin.play();
            ...
            && pipe3.getX() + (pipe3.getWidth() / 2) < hog.getX()
            + hog.getWidth()) {
         addScore(1);
      ...
      return (pipe1.collides(hog) || pipe2.collides(hog) || pipe3
            .collides(hog));
   }
